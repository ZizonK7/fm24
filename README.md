...
세부 공식이나 하한선이 미정인 항목은 [남은 설계 항목](#draft-open-items)에 모았다.
...
키 자체도 중요하게 보며, 가안으로는 **185cm 미만이면 감점**하는 방식을 검토한다.

다만 키가 조금 작더라도 점프력, 몸싸움, 헤더 능력이 충분히 높다면 일부 상쇄할 수 있게 한다.
구체적인 감점과 상쇄 공식은 아직 미정이다.
...
역할별 세부 공식과 하한·감점 기준을 정한 뒤 전체 지표 체계를 확정한다.
...
기본 공격 무기는 크로스지만,
역발 풀백의 경우 슈팅 능력이 좋다면 별도의 공격 옵션으로 인정한다.
...
높은 수비 라인을 유지하기 때문에
주력과 순간속도가 지나치게 낮은 센터백은 큰 감점을 받는다.
...
**크로스, 드리블, 슈팅처럼 확실한 공격 무기 하나를 가진 선수**를 높게 평가한다.
...
| 하위 지수            |  비중 | 의미                              |
| ---------------- | --: | ------------------------------- |
| **AttackWeapon** | 30% | 크로스, 드리블, 슈팅 중 하나 이상의 확실한 공격 강점 |
...
우풀백의 공격 능력은 단순 평균보다
가장 강한 공격 방식 하나를 우선적으로 평가하는 안을 검토한다.
...
세 공격 유형 중 가장 높은 값을 `AttackWeapon`으로 사용하는 방식을 우선 검토한다.
...
반사신경, 1대1, 핸들링, 위치선정 등을 본다.

발밑이 아무리 좋아도 기본 골키핑이 지나치게 낮은 선수는
높은 평가를 받지 않도록 하한선을 두는 방식을 검토한다.
...
위 가안의 역할 철학과 작성된 가중치는 유지하되, 아래 사항은 구현 전에 확정해야 한다.
이름이 같은 하위 지수도 역할마다 구성이 다를 수 있으므로 자동으로 같은 공식을 적용하지 않는다.

| 대상 | 미정 사항 |
| --- | --- |
| 완성형 포워드 | 185cm 미만의 신장 감점과 제공권에 의한 상쇄 공식 |
| 우윙 | 조율·찬스 생산·수비 책임감 하한 미달 시 감점 공식 |
| 레지스타 | `BallSecurity`, `Defence`의 세부 공식 |
| 수비형 미드필더 | `BuildUp`의 세부 공식, 185cm 신장 기준의 적용 방식 |
| 좌풀백 | `Defence`의 세부 공식, 역발 슈팅 옵션을 `Creation`에 반영하는 방식 |
| 좌센터백 | `Recovery`, `Aerial`, `BuildUp`의 세부 공식 또는 다른 역할과의 공유 여부 |
| 우센터백 | `Defence`, `Aerial`의 세부 공식 또는 다른 역할과의 공유 여부 |
| 센터백 공통 | 주력·순간속도 하한과 감점, 185cm 미만의 신장 감점 및 제공권 상쇄 공식 |
| 우풀백 | `Decision`, `Defence`의 세부 공식, 역발 득점형 선택에 필요한 주발·약발 조건 |
| 골키퍼 | `Composure`의 직접 능력치 사용 여부, `Goalkeeping`의 가중치와 하한·감점, 골킥/킥 능력의 데이터 매핑 |
